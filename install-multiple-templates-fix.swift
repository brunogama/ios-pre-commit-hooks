...
print("Pre-commit-configs Multiple Templates Fix")
print("=======================================")
...
let patchPath = "\(basePath)/Sources/Utils/MultipleTemplatesFix.swift"

// Get method code from MultipleTemplatesFix
...
let backupPath = "\(installerSourcePath).bak.multiple"
try installerContent.write(toFile: backupPath, atomically: true, encoding: .utf8)
print("✓ Created backup at: \(backupPath)")

// Find the original method implementation (either the original or our previous patch)
if let oldMethodRange = installerContent.range(of: "func updateConfigFileWithTemplates(selectedTemplates: [String]) throws {") {
    ...
    // Find and remove any existing patch extension if it exists
    if let extensionStartRange = patchedInstallerContent.range(of: "\n\n/// Installer service patch that") {
        // Find the extension end (last closing brace)
        if let extensionEndRange = patchedInstallerContent.range(of: "}\n}", options: .backwards) {
            let extensionEnd = patchedInstallerContent.index(extensionEndRange.upperBound, offsetBy: 0)
            patchedInstallerContent.removeSubrange(extensionStartRange.lowerBound..<extensionEnd)
        }
    }
...
print("Now run `./install.swift` to use the patched installer.")
print("To revert to the original file, use: mv \(backupPath) \(installerSourcePath)")
