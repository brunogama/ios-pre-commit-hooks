...
class InstallerService {
    ...
    func updateConfigFileWithTemplates(selectedTemplates: [String]) throws {
        ...
        for templateDir in selectedTemplates {
            ...
            if fileManager.fileExists(atPath: templatePath) {
                if let templateContent = try? String(contentsOfFile: templatePath, encoding: .utf8) {
                    // Skip the first line which is typically a comment with the name
                    let lines = templateContent.components(separatedBy: .newlines)
                    if lines.count > 1 {
                        let config = lines[1..<lines.count].joined(separator: "\n")
                        addedContent += "\n\(config)"
                    }
                }
            }
        ...
        // Append the configurations to the file
        if !addedContent.isEmpty {
            configContent += addedContent
        }
